...
2. 脚本只覆盖**确定性信号**(缺文件、未锁依赖、.env 被跟踪、绝对路径、行尾、git 状态)。
   ...
   - **日志与账本是否分离?** PROGRESS 一旦超过单次可读上限(Read 单次 ~25k tokens),"要求通读"
     **物理不可执行**,「下一步/待办」散落正文必被截断遗漏 → 新 session 只读开头就自行开工/宣布完成
     (实案:41k tokens 日志反复致漂)。修法:`BACKLOG.md` 唯一账本(<100 行:OPEN/常设裁定/CLOSED),
     开工先读账本,收尾登记/关闭,**宣布「完成/无缺口」前 OPEN 清零**。脚本已查确定性部分(超长且无账本)。
...
3. **核心步骤——填充真实内容**(这是 agent 的活,不是脚本的):
   ...
   - `BACKLOG.md`:把已知未完事项逐条登记进 OPEN(宁多勿漏);已定终态的裁定放「常设裁定」。
...
2. 更新 `PROGRESS.md`:在顶部追加一条本 session 记录(做了/状态/为什么/下一步/卡点);
   **并同步 `BACKLOG.md`**(若有):「下一步/待/暂缓」逐条登记 OPEN,完成项移 CLOSED 回写 commit 凭据。
...
- **日志可长,账本必短:** 待办永远只认 `BACKLOG.md`(<100 行);**别指望任何 agent 通读长日志** —— 超过单次可读上限后那是物理不可能,不是态度问题。
